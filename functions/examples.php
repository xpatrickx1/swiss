...
function my_register_taxonomy() {
    ...
    register_taxonomy( 'sample-category', 'examples', $args );
    ...
    register_taxonomy( 'types-category', 'examples', $args );
}
...
function wpse_358157_parse_request( $wp ) {
    $path      = 'examples'; // rewrite slug; no trailing slashes
    $taxonomy  = 'sample-category';        // taxonomy slug
    $post_type = 'examples';                 // post type slug
...
function set_posts_per_page_for_examples($query){
    if (!is_admin() && $query->is_main_query() && is_tax('sample-category')) {
        $query->set('posts_per_page', 1);
    }
}

add_action('pre_get_posts', 'set_posts_per_page_for_examples');

function examples_short_excerpt($excerpt)
{
    global $post;
    return substr($excerpt, 0, 229) . '...';
}

add_filter('the_excerpt', 'examples_short_excerpt');